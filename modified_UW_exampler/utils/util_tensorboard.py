...
def histogram_logger(tb_writer, iter_index, net=None, net_name=None):
    if net is not None:
        for tag, value in net.named_parameters():
            tag = tag.replace('.', '/')
            tb_writer.add_histogram(net_name + '_' + tag, to_np(value), iter_index)
            tb_writer.add_histogram(net_name + '_' + tag + '/grad', to_np(value.grad), iter_index)

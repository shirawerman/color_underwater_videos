...
from utils.util_distortion import (CenterPad_threshold, Normalize, RGB2Lab,
                                   ToTensor)
from utils.util_tensorboard import TBImageRecorder, value_logger, histogram_logger
from utils.warping import WarpingLayer

cv2.setNumThreads(0)

parser = argparse.ArgumentParser()
parser.add_argument("--data_root", default='../data_root/train_data') # "/home/zhanbo/remote/video/video_pair3/", type=str) ## DANA
parser.add_argument("--data_root_imagenet", default='../train_data/data_root/lioness/reference')  #"/home/zhanbo/remote/imagenet_ref_pair/", type=str)  ## DANA
parser.add_argument("--gpu_ids", type=str, default="0,1", help="separate by comma")  # "0,1,2,3" ## DANA
parser.add_argument("--workers", type=int, default=8)
parser.add_argument("--batch_size", type=int, default=15)
parser.add_argument("--image_size", type=int, default=[192, 384])
parser.add_argument("--ic", type=int, default=9)
parser.add_argument("--epoch", type=int, default=1)  # 40  ## DANA
...
parser.add_argument("--lr_step", type=int, default=750)
...
parser.add_argument("--save_net_step", type=int, default=2000)
...
def image_logger_fn(
    ...
    I_last_uw_image = batch_lab2rgb_transpose_mc(I_last_uw_lab[0:32, 0:1, :, :], I_last_uw_lab[0:32, 1:3, :, :])
    I_current_uw_image = batch_lab2rgb_transpose_mc(I_current_uw_lab[0:32, 0:1, :, :], I_current_uw_lab[0:32, 1:3, :, :])
    I_last_gt_image = batch_lab2rgb_transpose_mc(I_last_gt_lab[0:32, 0:1, :, :], I_last_gt_lab[0:32, 1:3, :, :])
    I_current_gt_image = batch_lab2rgb_transpose_mc(I_current_gt_lab[0:32, 0:1, :, :], I_current_gt_lab[0:32, 1:3, :, :])
    ...
    img_info = {}
    img_info["1_I_uw_last"] = I_last_uw_image
    img_info["2_I_uw_current"] = I_current_uw_image
    img_info["3_I_gt_last"] = I_last_gt_image
    img_info["4_I_gt_current"] = I_current_gt_image
    img_info["5_I_reference"] = I_reference_image
    img_info["6_I_last_predict"] = I_last_image_predict
    img_info["7_I_curren_predict"] = I_current_image_predict
    img_info["8_I_last_nonlocal"] = I_last_nonlocal_image
    img_info["9_I_current_nonlocal"] = I_current_nonlocal_image
    ...
    try:
        if total_iter % opt.print_step == 0:
            ...
            print(
                ...
                # "Epoch %d, Step[%d/%d], total_loss: %.2f"
                % (
                    epoch,
            ...
            print(f"grad: {colornet.module.conv1_1[0].weight.grad.mean()}")
            ...
            histogram_logger(tb_writer, total_iter, net=colornet, net_name='colornet')
            histogram_logger(tb_writer, total_iter, net=nonlocal_net, net_name='nonlocal_net')

        if total_iter % opt.tb_log_step == 0:
            I_last_nonlocal_lab = torch.cat(
                (I_last_uw_lab[:, 0:1, :, :], I_last_nonlocal_lab_predict[:, 1:3, :, :]), dim=1
            )
            I_current_nonlocal_lab = torch.cat(
                (I_current_uw_lab[:, 0:1, :, :], I_current_nonlocal_lab_predict[:, 1:3, :, :]), dim=1
            )
            I_last_lab_predict = torch.cat((I_last_uw_l, I_last_ab_predict), dim=1)
            data_queue.put(
                (
                    (
                        I_last_uw_lab.cpu(),
                        I_current_uw_lab.cpu(),
                        I_last_gt_lab.cpu(),
                        I_current_gt_lab.cpu(),
                        I_reference_lab.cpu(),
        ...
        if total_iter % opt.save_net_step == 0 or total_iter % 28818 == 0:
...
def load_data():
    ...
    assert imagenet_training_length == 0, "imagenet train size should be 0"
...
def define_optimizer():
    ...
    optimizer_g = optim.Adam(
        # [{"params": nonlocal_net.parameters(), "lr": 1e-6}, {"params": colornet.parameters(), "lr": 2e-5}],
    ...
    optimizer_d = optim.Adam(
        # filter(lambda p: p.requires_grad, discriminator.parameters()), lr=2 * 1e-5, betas=(0.5, 0.999)

        filter(lambda p: p.requires_grad, discriminator.parameters()), lr=2 * 1e-4, betas=(0.5, 0.999)
        # filter(lambda p: p.requires_grad, discriminator.parameters()), lr=opt.lr, betas=(0.5, 0.999)
    )
...
def resume_model():
    ...
    ########################################################## Shira- patch to deal with loading state dict module issue
    nonlocal_net_state_dict = checkpoint["nonlocal_net_state"]
    nonlocal_net_state_dict_new = {}
    for key, value in nonlocal_net_state_dict.items():
        new_key = key
        if "module." == key[0:7]:
            new_key = key[7:]
        nonlocal_net_state_dict_new[new_key] = value

    colornet_state_dict = checkpoint["colornet_state"]
    colornet_state_dict_new = {}
    for key, value in colornet_state_dict.items():
        new_key = key
        if "module." == key[0:7]:
            new_key = key[7:]
        colornet_state_dict_new[new_key] = value
        
    discriminator_state_dict = checkpoint["discriminator_state"]
    discriminator_state_dict_new = {}
    for key, value in discriminator_state_dict.items():
        new_key = key
        if "module." == key[0:7]:
            new_key = key[7:]
        discriminator_state_dict_new[new_key] = value
    ##########################################################
    
    nonlocal_net.load_state_dict(nonlocal_net_state_dict_new) #checkpoint["nonlocal_net_state"]
    discriminator.load_state_dict(discriminator_state_dict_new) #checkpoint["discriminator_state"]
    colornet.load_state_dict(colornet_state_dict_new) #checkpoint["colornet_state"]
...
def video_colorization():
    ...
    I_last_ab_predict, I_last_nonlocal_lab_predict, features_last_uw = frame_colorization(
        I_last_uw_lab,
        I_reference_lab,
        placeholder_lab,
        features_B,
    ...
    I_current_ab_predict, I_current_nonlocal_lab_predict, features_current_uw = frame_colorization(
        I_current_uw_lab,
        I_reference_lab,
        I_last_lab_predict,
        features_B,
...
if __name__ == "__main__":
    ...
    if opt.load_pretrained_model:
        ...
        discriminator_pretain_path = os.path.join("checkpoints/video_moredata_l1/", "discriminator_iter_76000.pth")
        discriminator.load_state_dict(torch.load(discriminator_pretain_path))

        rand_init_of_conv_layer = nn.Conv2d(9, 32, 3, 1, 1)
        rand_init_of_conv_layer.weight.data[:, 2:9, :, :] = colornet.conv1_1[0].weight.data
        rand_init_of_conv_layer.bias.data = colornet.conv1_1[0].bias.data
        colornet.conv1_1[0] = rand_init_of_conv_layer
    ...
    for iter, data in enumerate(data_loader):
        ...
        print(f"epoch {epoch}, iter {iter} out of {iter_num_per_epoch}")
        ...
        # to device
        I_last_uw_lab = I_last_uw_lab.to(device)
        I_current_uw_lab = I_current_uw_lab.to(device)
        I_last_gt_lab = I_last_gt_lab.to(device)
        I_current_gt_lab = I_current_gt_lab.to(device)
        ...
        # split lab images to 'l' and 'ab'
        I_last_uw_l = I_last_uw_lab[:, 0:1, :, :]
        I_last_uw_ab = I_last_uw_lab[:, 1:3, :, :]
        I_last_gt_l = I_last_gt_lab[:, 0:1, :, :]
        I_last_gt_ab = I_last_gt_lab[:, 1:3, :, :]

        I_current_uw_l = I_current_uw_lab[:, 0:1, :, :]
        I_current_uw_ab = I_current_uw_lab[:, 1:3, :, :]
        I_current_gt_l = I_current_gt_lab[:, 0:1, :, :]
        I_current_gt_ab = I_current_gt_lab[:, 1:3, :, :]
        ...
        if opt.weight_gan > 0:
            ...
            fake_data_lab = torch.cat(
                (uncenter_l(I_current_uw_l), I_current_ab_predict, uncenter_l(I_last_uw_l), I_last_ab_predict), dim=1
            )
            real_data_lab = torch.cat((uncenter_l(I_current_gt_l), I_current_gt_ab, uncenter_l(I_last_gt_l), I_last_gt_ab), dim=1)
        ...
        I_predict_rgb = tensor_lab2rgb(torch.cat((uncenter_l(I_current_uw_l), I_current_ab_predict), dim=1))
        ...
        I_current_rgb = tensor_lab2rgb(torch.cat((uncenter_l(I_current_gt_l), I_current_gt_ab), dim=1))
        ...
        if opt.weigth_l1 > 0:
            ...
            l1_loss = weighted_l1_loss(I_current_ab_predict, I_current_gt_ab, sample_weights) * opt.weigth_l1
        ...
        if opt.weight_smoothness > 0:
            scale_factor = 1
            I_current_lab_predict = torch.cat((I_current_uw_l, I_current_ab_predict), dim=1)
            IA_ab_weighed = weighted_layer_color(
                I_current_gt_lab, I_current_lab_predict, patch_size=3, alpha=10, scale_factor=scale_factor
            )
            smoothness_loss = (
